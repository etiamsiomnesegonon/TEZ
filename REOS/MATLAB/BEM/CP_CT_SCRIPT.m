...
rho = 1.225;             % density of sea water (kg/m^3)
U0 = 8; %2.77;                % streamwise current (m/s)
Blades = 3;               % number of blades

Pitch= 0 ; %0.1;              % operational pitch angle (deg)
...
load ESPE21_Turbine % ESPE's FX21. contain Chord, twist, position

% Convert to Gabriel's variable names
rad = RElm;
B = deg2rad(Twist);
c = Chord;
Blades = 3;

NBsec = 50; %100 % number of blade sections

r=linspace(rad(1),rad(end),NBsec);
% cubic spline for blade twist and chord
B=interp1(rad,B,r,'PCHIP')+pitch; % B is twist
c=interp1(rad,c,r,'PCHIP');       % c is chord length
...
% loop TSR
TSR=0.5:0.25:14;               % tip speed ratio


%load S814_360_BEM_data % AEROFOIL DATA CORRECTED EXTRAPOLATED FOR DEEP STALL
load S809_360_BEM_data % AEROFOIL DATA CORRECTED EXTRAPOLATED FOR DEEP STALL from QBLADE
CD = CD' ; Alpha = Alpha'; CL = CL';
...
plot(ax1,TSR,CP,'-.',TSR_Adyn,CP_Adyn,'k:',x1,y1,'r--')
ylabel('$$C_P$$')
xlabel('$$\lambda$$')
grid off
axis([0 15 0 0.5])
...
plot(ax2,TSR,CT,'-.',TSR_Adyn,CT_Adyn,'k:',x2,y2,'r--')
ylabel('$$C_T$$')
xlabel('$$\lambda$$')
